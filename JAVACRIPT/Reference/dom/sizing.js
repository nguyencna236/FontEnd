...
log(boxed.clientLeft); //  vị trí của nó so với bên trái border( độ rộng của border)
...
log(window.innerHeight); // lấy chiều dài của trang
log(window.outerHeight); // lấy chiều cao của cả màng hình máy tính
log(window.innerWidth);
log(window.outerWidth);

// 4. selector.getBoundingClientRect() -> lấy ra toạ độ, kích thước của phần tử
log(boxed.getBoundingClientRect()); // lấy ra đám bên dưới
...
log(li);
log(li2);
// Điểm giống: có thể truy cập bằng index, có độ dài(length), giống mảng nhưng ko hẳn là mảng 
//tức là không sử dụng được những phương thức đã học như là pop, shift, push, map, filter
